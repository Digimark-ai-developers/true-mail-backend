...
async def batch_post(
    ...
    if email_column not in df.columns:
        return templates.TemplateResponse("batch.html", {"request": request, "columns": columns, "selected_column": email_column, "error": "Selected column not found."})
    ...
    def validate_email(i, email):
        if not validate_email_syntax(str(email)):
            ...
            is_valid, message = check_email_reachability(str(email), sender_email, disposable_domains)
    ...
    df['Validation_Message'] = df[email_column].map(lambda x: email_result_map.get(str(x), {}).get('Validation_Message', ''))

...
class EmailService:
    ...
    async def create_test_email(self, user_id: str, test_email: SimpleEmailCheckRequest, test_id: str, sender_email: str = "test@example.com"):
        try:
            # Validate User
    ...
    def process_bulk_email_upload(task_id: str, user_id: str, file_content: str, file_name: str, db: Session):
        ...
        try:
            ...
            bulk_email_status_cache[task_id] = {"status": "completed", "message": "Bulk emails processed successfully.", "result": result.dict()}
        except Exception as e:
            bulk_email_status_cache[task_id] = {"status": "failed", "message": "Bulk email processing failed.", "error": str(e)}
